    ...
    <div className="animate-fade-in relative">
      ...
      {productToDelete && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-primary/40 backdrop-blur-sm transition-all duration-300">
          <div className="bg-background rounded-xl shadow-2xl p-8 max-w-md w-full animate-fade-up">
            <h3 className="text-xl font-serif text-foreground mb-2">Delete Product</h3>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
        <div>
          <h1 className="text-3xl font-serif tracking-tight text-foreground mb-2">Products</h1>
          <p className="text-sm text-stone-500 font-normal">Manage your inventory, prices, and product details.</p>
        </div>
        <Link 
          href="/admin/add-product" 
          className="bg-primary text-white px-6 py-3 text-[11px] font-medium uppercase tracking-[0.2em] rounded-md hover:bg-stone-800 transition-colors shadow-[0_2px_10px_-4px_rgba(0,0,0,0.5)]"
        >
          Add Product
        </Link>
      </div>
      ...
      {error ? (
        ...
        <div className="bg-background rounded-xl border border-stone-300/60 shadow-[0_2px_8px_-4px_rgba(0,0,0,0.05)] overflow-hidden flex flex-col">
          
          {/* Filters Bar */}
          <div className="p-6 border-b border-stone-200 bg-background-alt/30 flex flex-col md:flex-row gap-4 items-center">
            {/* Search Name */}
            <div className="relative w-full md:flex-1">
              <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-stone-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-background border border-stone-300/80 rounded-lg text-sm outline-none focus:ring-1 focus:ring-stone-900 focus:border-stone-900 transition-all font-normal placeholder:text-stone-500 shadow-sm"
              />
            </div>
            ...
            <div className="w-full md:w-48 relative">
              <select
                ...
                className={`appearance-none w-full px-4 py-2.5 bg-background border border-stone-300/80 rounded-lg text-sm outline-none focus:ring-1 focus:ring-stone-900 focus:border-stone-900 transition-all font-normal shadow-sm ${categoryFilter ? 'text-foreground' : 'text-stone-500'}`}
              >
            ...
            <div className="w-full md:w-auto flex items-center gap-2">
              <div className="relative">
                 <span className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-500 font-medium text-xs">₹</span>
                 <input 
                  type="number" 
                  step="0.01"
                  placeholder="Min" 
                  ...
                  className="w-24 pl-6 pr-3 py-2.5 bg-background border border-stone-300/80 rounded-lg text-sm outline-none focus:ring-1 focus:ring-stone-900 focus:border-stone-900 transition-all font-normal placeholder:text-stone-500 shadow-sm"
                />
              </div>
            ...
            {(searchQuery || categoryFilter || minPrice || maxPrice) && (
              <button 
                ...
                className="text-[10px] uppercase tracking-widest text-stone-500 hover:text-foreground transition-colors whitespace-nowrap ml-2 font-medium bg-stone-100 hover:bg-stone-200 px-4 py-2.5 rounded-md"
              >
                Clear
              </button>
            )}
          ...
          <div className="overflow-x-auto min-h-[400px]">
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-stone-100 bg-background">
                {loading ? (
                  <tr>
                    <td colSpan={6} className="px-8 py-16 text-center text-stone-500 font-normal text-base">Loading product catalog...</td>
                  </tr>
                ) : filteredProducts.length > 0 ? (
                  filteredProducts.map((p) => {
                    const id = p._id || p.id;
                    return (
                      <tr key={id} className="hover:bg-background-alt/50 transition-colors group">
                        <td className="px-8 py-4">
                          <div className="w-12 h-12 rounded-lg bg-stone-100 overflow-hidden border border-stone-300/60 shadow-sm">
                            {p.image ? (
                        ...
                        <td className="px-8 py-4 font-medium text-foreground tracking-tight">{p.name}</td>
                        ...
                        <td className="px-8 py-4 text-right">
                          <div className="flex items-center justify-end gap-4 h-full">
                            <Link 
                              ...
                              className="text-[10px] font-medium uppercase tracking-widest text-[#888888] hover:text-foreground transition-colors"
                            >
                              Edit
                            </Link>
                            <button 
                  ...
                  <tr>
                    <td colSpan={6} className="px-8 py-16 text-center text-stone-500 font-normal">
