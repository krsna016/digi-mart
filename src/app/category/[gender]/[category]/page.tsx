...
function CategoryContent() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${BASE_URL}/products?gender=${gender}&category=${category}`, {
          signal: controller.signal
        });
        if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
        const data = await res.json();
        
        // Filter strictly by both gender and category
    ...
    if (gender && category) fetchProducts();
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#FCFBF8]">
      ...
      {/* Sort */}
      <div className="max-w-[1400px] mx-auto w-full px-6 lg:px-12 pt-12 pb-6 flex justify-end">
