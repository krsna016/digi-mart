        ...
        <div className="relative mx-auto max-w-[1440px] px-8 lg:px-12 border-b border-stone-200/50">
          ...
          <div className="flex h-16 lg:h-20 items-center justify-between">
            ...
            <div className="flex-1 flex items-center justify-end gap-3 lg:gap-6 pt-1">
                  ...
                  {mounted && isAuthenticated && (
                    <div className="py-2">
                      <div className="px-6 py-4 border-b border-stone-50">
                        ...
                        <p className="text-[20px] font-medium text-stone-900 truncate font-serif">{user?.name || user?.email}</p>
                      </div>
                      <div className="py-2 border-b border-stone-50">
                        <Link href="/account" onClick={() => setIsAccountOpen(false)} className="flex items-center gap-3 px-6 py-3 text-[11px] font-bold uppercase tracking-[0.2em] text-stone-700 hover:text-stone-900 hover:bg-stone-50/50 transition-all group">My Account</Link>
                        <Link href="/orders" onClick={() => setIsAccountOpen(false)} className="flex items-center gap-3 px-6 py-3 text-[11px] font-bold uppercase tracking-[0.2em] text-stone-700 hover:text-stone-900 hover:bg-stone-50/50 transition-all group">Orders</Link>
                        {user?.role === 'admin' && (
                          <Link href="/admin" onClick={() => setIsAccountOpen(false)} className="flex items-center gap-3 px-6 py-3 text-[11px] font-bold uppercase tracking-[0.2em] text-stone-900 hover:bg-stone-50/50 transition-all group">Admin Dashboard</Link>
                        )}
                      </div>
                      <div className="pt-2">
              ...
              <Link href="/wishlist" className="relative p-1.5 text-stone-600 hover:text-stone-900 transition-all duration-300 hover:scale-105">
                <Heart className={`w-5 h-5 transition-colors ${mounted && wishlistCount > 0 ? 'fill-stone-900 text-stone-900' : ''}`} strokeWidth={1.2} />
                {mounted && wishlistCount > 0 && <span className="absolute top-0 right-0 w-3.5 h-3.5 bg-stone-900 text-white text-[8px] font-bold flex items-center justify-center rounded-full border border-white animate-pulse">{wishlistCount}</span>}
              </Link>
              <Link href="/cart" className="relative p-1.5 text-stone-600 hover:text-stone-900 transition-all duration-300 hover:scale-105">
                <ShoppingBag className="w-5 h-5" strokeWidth={1.2} />
                {mounted && cartCount > 0 && <span className="absolute -top-0.5 -right-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-stone-900 text-[8px] font-bold text-white ring-2 ring-white">{cartCount > 9 ? '9+' : cartCount}</span>}
              </Link>
            </div>
          </div>
            ...
            <nav className="flex gap-12 items-center justify-center">
              {['men', 'women', 'kids'].map((mainCat) => (
                <div key={mainCat} className="h-full flex items-center relative" onMouseEnter={() => setHoveredCategory(mainCat)} onMouseLeave={() => setHoveredCategory(null)}>
                  <Link href={`/category/${mainCat}`} className="relative h-full flex items-center py-2 text-[11px] font-bold uppercase tracking-[0.25em] text-stone-600 hover:text-stone-900 transition-all duration-500 group">
                    {mainCat}
                          ...
                          <div className="col-span-9 p-14 pr-8">
                            <div className="flex flex-col gap-12">
                              <div className="flex flex-col gap-1.5">
                                <p className="text-[10px] font-bold uppercase tracking-[0.5em] text-stone-400">Essential Edit</p>
                                <h3 className="text-5xl font-serif text-stone-900 capitalize tracking-tight">The {mainCat}&rsquo;s Collection</h3>
                              </div>
                                ...
                                {categories[mainCat] && Object.entries(categories[mainCat]).map(([group, items]: [string, any]) => (
                                  <div key={group} className="flex flex-col gap-6">
                                    <div className="flex flex-col gap-2">
                                      <span className="text-[13px] font-bold uppercase tracking-[0.2em] text-stone-900">{group}</span>
                                      <div className="h-[1px] w-6 bg-stone-900/10" />
                                    </div>
                                    <div className="flex flex-col gap-3.5">
                                      {items.map((sub: string) => (
                                        <Link key={sub} href={`/category/${mainCat}/${sub.toLowerCase().replace(/\s+/g, '')}`} className="group/sub text-[15px] font-normal text-stone-500 hover:text-stone-900 transition-all duration-300 flex items-center justify-between" onClick={() => setHoveredCategory(null)}>
                            ...
                            <div className="mt-16 pt-8 border-t border-stone-100 flex items-center justify-between">
                              <Link href={`/category/${mainCat}`} className="text-[11px] font-bold uppercase tracking-[0.3em] text-stone-900 flex items-center gap-2 group/all transition-all duration-300" onClick={() => setHoveredCategory(null)}>View full {mainCat} collection <ArrowRight className="w-3.5 h-3.5 group-hover/all:translate-x-1 transition-transform" strokeWidth={2} /></Link>
                              <div className="flex gap-8 text-[10px] font-medium uppercase tracking-[0.2em] text-stone-400"><span>Free Shipping Over ₹150</span><span>Easy Returns</span></div>
                            </div>
                          </div>
                          <div className="col-span-3 relative group/banner overflow-hidden border-l border-stone-100/50">
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            className="fixed inset-0 z[60] bg-[#FCFBF8] lg:hidden flex flex-col h-full"
          >
            <div className="flex items-center justify-between p-8 border-b border-stone-100 mt-4">
              <Link href="/" className="text-2xl font-serif text-stone-900" onClick={() => setMobileMenuOpen(false)}>DIGIMART</Link>
              <button onClick={() => setMobileMenuOpen(false)}><X className="w-6 h-6 text-stone-900" strokeWidth={1.5} /></button>
            </div>
              ...
              <div className="space-y-12">
                {['men', 'women', 'kids'].map((mainCat) => (
                  <div key={mainCat} className="space-y-6">
                    ...
                    >
                      <span className="text-4xl font-serif text-stone-900 capitalize">{mainCat}</span>
                      <ArrowRight className={`w-6 h-6 text-stone-400 transition-transform duration-500 ${expandedCategory === mainCat ? 'rotate-90' : ''}`} />
                    </button>
                    {expandedCategory === mainCat && (
                      <div className="space-y-10 pl-4 border-l border-stone-100">
                        {categories[mainCat] && Object.entries(categories[mainCat]).map(([group, items]: [string, any]) => (
                          <div key={group} className="space-y-4">
                            <p className="text-[11px] font-bold uppercase tracking-widest text-stone-400">{group}</p>
              ...
              <div className="mt-16 pt-12 border-t border-stone-100">
                {isAuthenticated ? (
                  <div className="space-y-6">
                    <Link href="/account" className="block text-lg font-medium text-stone-900" onClick={() => setMobileMenuOpen(false)}>My Account</Link>
                    <button onClick={() => { logout(); setMobileMenuOpen(false); }} className="block text-lg font-medium text-red-500">Logout</button>
                  </div>
                ) : (
                  <Link href="/login" className="block text-lg font-medium text-stone-900" onClick={() => setMobileMenuOpen(false)}>Sign In</Link>
                )}
              </div>
            </div>
          </motion.div>
