...
export default function CategorySection() {
  return (
    <section className="w-full bg-background-alt py-20 rounded-[2.5rem] shadow-premium border border-stone-200/40">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          <div>
            ...
            <h2 className="text-4xl md:text-5xl font-serif text-foreground tracking-tight leading-tight">Shop by Category</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {CATEGORIES.map((cat, idx) => (
            <Link 
              key={idx} 
              href={cat.link}
              className="group relative block w-full aspect-[4/5] overflow-hidden rounded-[2rem] shadow-lg border border-stone-200/60 bg-stone-100"
            >
              <img 
                src={cat.image} 
                alt={cat.title} 
              ...
              />
              <div className="absolute inset-0 bg-primary/10 transition-opacity duration-500 group-hover:bg-primary/40" />
              
              <div className="absolute inset-0 p-8 flex flex-col justify-end">
                <div className="bg-background/95 backdrop-blur-sm p-6 rounded-[2.5rem] transform translate-y-4 opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
